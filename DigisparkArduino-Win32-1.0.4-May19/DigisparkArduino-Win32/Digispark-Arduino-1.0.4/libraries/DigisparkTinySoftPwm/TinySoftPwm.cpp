...
#define TINY_SOFT_PWM_SET_PIN(RamIdx)	(PortPwmTo1 |= _BV(RamIdx))

#define TINY_SOFT_PWM_DECLARE_PIN(Px)		TINY_SOFT_PWM_DDR |= (1<<(Px)); PortPwmMask |= (1<<(Px))

#define TINY_SOFT_PWM_UNDECLARE_PIN(Px)		TINY_SOFT_PWM_DDR &= ~(1<<(Px)); PortPwmMask &= ~(1<<(Px))
...
static   uint8_t PortPwmMask=0x1A;
...
uint8_t RamIdx;
  ...
  */
  PwmOrder[PinIdx] = PwmToPwmMax(Pwm);
}
...
#endif
#if (TINY_SOFT_PWM_CH_MAX >= 2)
  if(Compare[1] == OvfCount) TINY_SOFT_PWM_SET_PIN(1);
#endif
#if (TINY_SOFT_PWM_CH_MAX >= 3)
  if(Compare[2] == OvfCount) TINY_SOFT_PWM_CLEAR_PIN(2);
#endif
#if (TINY_SOFT_PWM_CH_MAX >= 4)
  if(Compare[3] == OvfCount) TINY_SOFT_PWM_SET_PIN(3);
#endif
#if (TINY_SOFT_PWM_CH_MAX >= 5)
  if(Compare[4] == OvfCount) TINY_SOFT_PWM_SET_PIN(4);
#endif
#if (TINY_SOFT_PWM_CH_MAX >= 6)
...
void TinySoftPwm_unassignpin(void)
{
  //cli();
  TINY_SOFT_PWM_UNDECLARE_PIN(3);
  TINY_SOFT_PWM_UNDECLARE_PIN(4);
}

void TinySoftPwm_reassignpin(void)
{
  //cli();
